...
namespace RPCDapr.Server.Kestrel.Implements
{
    internal class RequestDelegate<Tobj, Tin, Tout> : BaseRequestDelegate where Tin : class, new() where Tout : class
    {
        ...
        {
            RPCDaprIocContainer.BuilderIocContainer(scope);//仅在当前请求内创建上下文模型
            ...
            try
            {
                ...
                HttpContextExtension.ContextWapper.Value = new RPCDaprHttpContextWapper(Path, scope, ctx);
            ...
            finally
            {
                ...
                RPCDaprIocContainer.DisposeIocContainer();//注销上下文
            }
        }
    }
}
