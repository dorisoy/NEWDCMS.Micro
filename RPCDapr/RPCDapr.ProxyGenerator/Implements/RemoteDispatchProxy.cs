...
namespace RPCDapr.ProxyGenerator.Implements
{
    ...
    public abstract class RemoteDispatchProxyBase : DispatchProxy
    {
        ...
        {
            return (IRemoteMessageSenderDelegate)Activator.CreateInstance(typeof(RemoteMessageSenderDelegate<,>).MakeGenericType(inputType ?? typeof(object), methodInfo.ReturnType), methodInfo, RPCDaprIocContainer.Resolve<IRemoteMessageSender>());
        }
    }
}
