...
namespace Domain.Services
{
    ...
    {
        Func<IEnumerable<int>, Task<List<OrderGoodsSnapshot>>> getGoodsList;
        ...
        public async Task<Order> CreateOrder(int userId, string consigneeName, string consigneeAddress, string consigneeTel, List<OrderItem> orderItems)
        {
            var order = new Order();
    ...
    public partial class CreateOrderService
    {
        ...
        public async Task<Order> FinalCreateOrder(int userId, string consigneeName, string consigneeAddress, string consigneeTel, List<OrderItem> orderItems)
        {
            var order = new Order();
            if (orderItems == null || !orderItems.Any())
                throw new DomainException("订单明细不能为空!");
            ...
            {
                var goods = goodslist.FirstOrDefault(x => x.GoodsId == item.GoodsId);
                //由于这里不使用actor，所以校验工作在DTPDapr的上一个流程已经处理完毕了，此处仅需要进行实体明细填充
                item.Create(order.Id, goods);
            }
            //创建订单实体
            order.CreateOrder(userId, consigneeName, consigneeAddress, consigneeTel, orderItems);
