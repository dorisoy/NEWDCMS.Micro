...
namespace Infrastructure.Repository
{
    public class AccountRepository : RepositoryBase<EfDbContext, Domain.Entities.Account, Account>, Domain.Repository.IAccountRepository
    {
        ...
        public async Task<bool> CheckAcccountUserInfoAny(int accountId)
        {
        ...
        public async Task<List<string>> GetAccountPermissions(int accountId)
        {
        ...
        async Task FillAccountInfo(Domain.Entities.Account account)
        {
            if (account != null)
            {
                ...
                account.Roles = await context.UserRole.AsNoTracking().Where(x => x.AccountId == account.Id).Select(x => x.RoleId).ToListAsync() ?? new List<int>();
            }
        }
    }
}
